      ...
      while ((he = hash_iterator_next (&hi)))
	{
	  ...
	  if (mi != sender_instance && !mi->halt)
	      ...
	      if (sender_addr)
		{
	      ...
	      if (vid != 0 && vid != mi->context.options.vlan_vid)
		continue;
#endif
	      multi_add_mbuf (m, mi, mb);
	    }
...
remove_vlan_identifier (struct buffer *buf)
{
  ...
  if (ntohs (vlanhdr.tpid) != OPENVPN_ETH_P_8021Q)
    {
      ...
      msg (D_VLAN_DEBUG, "dropping untagged frame (proto/len 0x%04x)",
	   ntohs (vlanhdr.tpid));
      goto err;
    }
  ...
  msg (D_VLAN_DEBUG, "untagging frame: vid: %d, wrapped proto/len: 0x%04x",
...
multi_prepend_vlan_identifier (struct multi_instance *mi, struct buffer *buf)
{
  ...
  vlanhdr_set_vid (&vlanhdr, htons (mi->context.options.vlan_vid));
  ...
  msg (D_VLAN_DEBUG, "tagging frame: vid %d (wrapping proto/len: %04x)",
       mi->context.options.vlan_vid, vlanhdr.proto);
}
